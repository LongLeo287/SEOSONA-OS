# Architecture Extract: EasySpider
...
EasySpider/
    ...
    .temp_to_pub/
        ...
        EasySpider_MacOS/
            .gitignore
            First time open please see this (The package is damaged and cannot verify the developer's solution).txt
            first_time_run.sh
            If chrome exits when designing a task, see this.txt
            LICENSE
            myCode.py
            readme.txt
            浏览器闪退解决方案（点击设计任务后Chrome弹出后立马退出）.txt
            第一次打开请看这个（包已损坏，无法验证开发者解决方案）.txt
            软件使用说明.txt
            Sample Tasks/
                0. Note - This folder is not the task storage directory for the EasySpider software under MacOS.txt
                0. 注意 - 此文件夹并不是MacOS系统的EasySpider软件的任务存储目录，只是示例任务文件.txt
                0.json
                1.json
                10.json
                100.json
        ...
        EasySpider_windows_x64/
            ...
            tasks/
                ...
                99.json
    Docs/
        面向WEB应用的智能化服务封装系统设计与实现.pdf
    ElectronJS/
        .gitignore
        change_version.py
        check_update.js
        chrome_location_linux
        ...
        EasySpider_zh/
            ...
            style/
                bootstrap.min.css
                toolkit.css
        selenium-webdriver-bidi-webExtension/
            extensionData.js
            webExtension.js
        src/
            index.html
    ...
    Examples/
        ...
        汽车新闻采集/
            第一步：采集列表超链接地址.json
            第二步：采集详情页标题内容并下载所有图片.json
    ExecuteStage/
        .gitignore
        config.json
        constants.py
        easyspider_executestage.py
        easyspider_executestage_single.py
        favicon.ico
        fl_beta.py
        generateExecutable_Linux64.sh
        generateExecutable_Macos.sh
        generateEXE_win32.cmd
        generateEXE_win64.cmd
        llm_beta.py
        myChrome.py
        myCode.py
        package_size.py
...
### `.temp_to_pub\EasySpider_Linux_x64\tasks\0.json`
```
...
### `.temp_to_pub\EasySpider_Linux_x64\tasks\101.json`
```
