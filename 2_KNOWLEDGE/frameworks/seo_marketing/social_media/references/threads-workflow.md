...
const authUrl = `http~/.seosona/path/?` +
  ...
  `redirect_uri=${encodeURIComponent(
    redirectUri.includes('https') ? redirectUri : `http~/.seosona/path/${redirectUri}`
...
const { access_token } = await fetch(
  `http~/.seosona/path/?grant_type=th_exchange_token&` +
  `client_secret=${THREADS_APP_SECRET}&access_token=${shortToken}`
).then(r => r.json());

// 4. Refresh token (before expiry)
const refreshed = await fetch(
  `http~/.seosona/path/?grant_type=th_refresh_token&access_token=${token}`
).then(r => r.json());
```
...
const user = await fetch(
  `http~/.seosona/path/?fields=id,username,threads_profile_picture_url&access_token=${token}`
).then(r => r.json());
```
...
const { id: contentId } = await fetch(
  `http~/.seosona/path/${userId}/threads`,
...
const { id: mediaId } = await fetch(
  `http~/.seosona/path/${userId}/threads?${params}`,
  { method: 'POST' }
).then(r => r.json());
```
...
for (const media of mediaArray) {
  ...
  const { id } = await fetch(`http~/.seosona/path/${userId}/threads?${params}`, { method: 'POST' }).then(r => r.json());
  mediaIds.push(id);
}
...
const { id: containerId } = await fetch(`http~/.seosona/path/${userId}/threads?${carouselParams}`, { method: 'POST' }).then(r => r.json());
```
...
async function checkMediaStatus(mediaContainerId, accessToken) {
  while (true) {
    const { status, error_message } = await fetch(
      `http~/.seosona/path/${mediaContainerId}?fields=status,error_message&access_token=${accessToken}`
    ).then(r => r.json());
...
const { id: threadId } = await fetch(
  `http~/.seosona/path/${userId}/threads_publish?creation_id=${contentId}&access_token=${accessToken}`,
  { method: 'POST' }
).then(r => r.json());

// Get permalink
const { permalink } = await fetch(
  `http~/.seosona/path/${threadId}?fields=id,permalink&access_token=${accessToken}`
).then(r => r.json());
```
...
const { id: replyThreadId } = await fetch(
  `http~/.seosona/path/${userId}/threads_publish?creation_id=${replyContentId}&access_token=${accessToken}`,
  { method: 'POST' }
).then(r => r.json());
```
...
const { data } = await fetch(
  `http~/.seosona/path/${userId}/threads_insights?` +
