...
const refreshed = await fetch('http~/.seosona/path/', {
  method: 'POST',
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  body: new URLSearchParams({
    client_key: TIKTOK_CLIENT_ID,
...
const { data: { user } } = await fetch(
  'http~/.seosona/path/?fields=open_id,avatar_url,display_name,union_id,username',
...
const { data: { publish_id } } = await fetch(
  `http~/.seosona/path/`, // or /inbox/video/init/ for UPLOAD
  {
    method: 'POST',
    headers: { 'Content-Type': 'application/json; charset=UTF-8', Authorization: `Bearer ${accessToken}` },
    body: JSON.stringify({
...
async function checkUploadStatus(publishId, accessToken, username) {
  while (true) {
    ...
    if (status === 'SEND_TO_USER_INBOX') {
      return { url: 'http~/.seosona/path/?tab=post', id: publishId };
    }
    if (status === 'PUBLISH_COMPLETE') {
      return {
        url: publicaly_available_post_id
          ? `http~/.seosona/path/@${username}/video/${publicaly_available_post_id[0]}`
          : `http~/.seosona/path/@${username}`,
