...
const tokenResponse = await fetch('http~/.seosona/path/', {
  method: 'POST',
...
// Refresh token
const refreshResponse = await fetch('http~/.seosona/path/', {
  method: 'POST',
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  body: new URLSearchParams({
...
```typescript
// 1. Initialize upload
const initResponse = await fetch(`http~/.seosona/path/?action=initializeUpload`, {
  method: 'POST',
  headers: {
...
await fetch('http~/.seosona/path/?action=finalizeUpload', {
  method: 'POST',
...
// Create post
const response = await fetch('http~/.seosona/path/', {
  method: 'POST',
  headers: {
    'LinkedIn-Version': '202511',
    'X-Restli-Protocol-Version': '2.0.0',
...
```typescript
await fetch(`http~/.seosona/path/${encodeURIComponent(postId)}/comments`, {
  method: 'POST',
...
const { elements } = await fetch(
  `http~/.seosona/path/?q=vanityName&vanityName=${vanity}`,
