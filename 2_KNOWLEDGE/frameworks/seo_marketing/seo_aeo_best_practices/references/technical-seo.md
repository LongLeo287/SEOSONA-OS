...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return pages.map(page => ({
    url: `http~/.seosona/path/${page.url}`,
...
export async function generateMetadata({ params }): Promise<Metadata> {
  return {
    alternates: {
      canonical: `http~/.seosona/path/${params.slug}`,
    },
  }
}
```
...
[Core Web Vitals](http~/.seosona/path/) impact rankings:
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> {
  ...
  return {
    alternates: {
      canonical: `http~/.seosona/path/${lang}/${slug}`,
      languages: {
        'en': `http~/.seosona/path/${slug}`,
        'de': `http~/.seosona/path/${slug}`,
        'x-default': `http~/.seosona/path/${slug}`,
      },
    },
  }
}
