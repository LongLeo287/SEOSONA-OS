...
```bash
# Measure vitals
node performance.js --url http~/.seosona/path/ | jq '.vitals'

# With trace for debugging
node performance.js --url http~/.seosona/path/ --trace trace.json
```
...
```bash
# Desktop (default)
node navigate.js --url http~/.seosona/path/
node performance.js --url http~/.seosona/path/
...
```bash
node network.js --url http~/.seosona/path/ | jq '
...
```bash
node screenshot.js --url http~/.seosona/path/ --full-page true \
...
```bash
node screenshot.js --url http~/.seosona/path/ \
...
node evaluate.js --url http~/.seosona/path/ --script "document.documentElement.outerHTML" \
  | jq -r '.result' > rendered.html

# Compare with source HTML
curl -s http~/.seosona/path/ > source.html
...
```bash
node evaluate.js --url http~/.seosona/path/ --script "
  JSON.stringify({
