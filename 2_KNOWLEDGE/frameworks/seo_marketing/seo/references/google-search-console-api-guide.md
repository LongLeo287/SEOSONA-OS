...
node scripts/gsc-query.cjs --top-queries -s http~/.seosona/path/
node scripts/gsc-query.cjs --low-ctr -s http~/.seosona/path/ -o report.csv -f csv
node scripts/gsc-query.cjs --inspect -s http~/.seosona/path/ -u /page
```
...
**Base URLs:**
- Search Analytics, Sitemaps, Sites: `http~/.seosona/path/`
- URL Inspection: `http~/.seosona/path/`
...
**Site URL formats:**
- URL-prefix: `http~/.seosona/path/` (include trailing slash)
- Domain property: `sc-domain:example.com` (no protocol)

**Authorization Scopes:**
- `http~/.seosona/path/` - Read-only access
- `http~/.seosona/path/` - Full read/write access
...
```json
{
  "inspectionUrl": "http~/.seosona/path/",
  "siteUrl": "http~/.seosona/path/",
  "languageCode": "en-US"
}
```
...
async function main() {
  ...
  const siteUrl = 'http~/.seosona/path/';
