...
curl -X POST "$BASE_URL/review" \
  ...
  -d '{"url": "http~/.seosona/path/", "instructions": "Focus on technical SEO"}'
```
...
curl -X POST "$BASE_URL/screenshot" \
  -d '{"url": "http~/.seosona/path/", "fullPage": true, "deviceType": "DESKTOP"}'

# Mobile
curl -X POST "$BASE_URL/screenshot" \
  -d '{"url": "http~/.seosona/path/", "fullPage": true, "deviceType": "MOBILE"}'
```

3. **Extract Links Map**
```bash
curl -X POST "$BASE_URL/scrape/links-map?url=http~/.seosona/path/" \
