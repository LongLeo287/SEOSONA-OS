...
ffmpeg -re -i input.mp4 \
  ...
  -pix_fmt yuv420p -g 60 -c:a aac -b:a 128k \
  -f flv rtmp~/.seosona/path/:443/rtmp/STREAM_KEY
```
...
```bash
# Sender
ffmpeg -re -i input.mp4 -c copy -f mpegts ud~/.seosona/path/:1234

# Receiver
ffplay ud~/.seosona/path/:1234
```
...
```bash
# Audio only
ffmpeg -re -i audio.mp3 -c:a libopus -f rtp rt~/.seosona/path/:5004

# Video + audio
ffmpeg -re -i input.mp4 \
  -c:v libx264 -preset ultrafast \
  -c:a aac -f rtp rt~/.seosona/path/:5004
```
...
```bash
# Reduce buffer size
ffmpeg -re -i input.mp4 -maxrate 2000k -bufsize 2000k -c:v libx264 -f flv rtm~/.seosona/path/

# Use faster preset
ffmpeg -re -i input.mp4 -preset ultrafast -c:v libx264 -f flv rtm~/.seosona/path/
```

### Audio/Video Desync
```bash
# Force constant frame rate
ffmpeg -re -i input.mp4 -r 30 -c:v libx264 -f flv rtm~/.seosona/path/

# Use -vsync 1
ffmpeg -re -i input.mp4 -vsync 1 -c:v libx264 -f flv rtm~/.seosona/path/
```

### Connection Drops
```bash
# Increase timeout
ffmpeg -timeout 5000000 -re -i input.mp4 -c:v libx264 -f flv rtm~/.seosona/path/

# Reconnect on failure (use wrapper script)
while true; do
  ffmpeg -re -i input.mp4 -c:v libx264 -f flv rtm~/.seosona/path/
  sleep 5
done
```
