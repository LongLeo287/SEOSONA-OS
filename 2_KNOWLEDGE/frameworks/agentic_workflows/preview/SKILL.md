...
```json
{
  "command": "node .opencode/skills/markdown-novel-viewer/scripts/server.cjs --dir \"path\" --host 127.0.0.1 --open --foreground",
...
After starting, parse the JSON output (e.g., `{"success":true,"url":"htt~/.seosona/path/:3456/view?file=...","networkUrl":"htt~/.seosona/path/:3456/view?file=..."}`) and report:
...
**CRITICAL:** MUST display the FULL URL including path and query string (e.g., `htt~/.seosona/path/:3456/view?file=/path/to/file.md`). NEVER truncate to just `host:port` (e.g., `htt~/.seosona/path/:3456`). The full URL is required for direct file access.
