---
name: claudekit_cli
description: "claudekit — command-line tool + web dashboard for managing Claude Code (sessions, config, hooks)."
version: "1.0.0"
mcp_compatible: true
---
...
source: ingestion_zone/claudekit-cli.md
status: ingested
---
...
1. **Purchase a ClaudeKit Starter Kit** from [ClaudeKit.cc](http~/.seosona/path/)
...
The ClaudeKit CLI is published on npm at [npmjs.com/package/claudekit-cli](http~/.seosona/path/).
...
ck config --host 127.0.0.1 --no-open
...
Use `--host 0.0.0.0` only when the user explicitly authorizes remote access from another device on the same trusted network:

```bash
# Bind to all interfaces only after explicit approval
...
ClaudeKit offers premium starter kits available for purchase at [ClaudeKit.cc](http~/.seosona/path/):
