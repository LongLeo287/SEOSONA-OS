...
| `--host <addr>` | Host to bind (`0.0.0.0` only for explicitly authorized remote access) | localhost |
...
Default to `127.0.0.1` for local use. When the user explicitly authorizes remote access and you use `--host 0.0.0.0`, the server auto-detects your local network IP:

```json
{
  "success": true,
  "url": "htt~/.seosona/path/:3500/kanban?dir=...",
  "networkUrl": "htt~/.seosona/path/:3500/kanban?dir=...",
  "port": 3500
}
```

Use `networkUrl` to access from other devices on the same network only after the user has approved LAN exposure.
...
**Remote access denied**: Use `--host 0.0.0.0` only after explicit user approval to bind all interfaces
