...
1. **Extract Video Data**
   ...
   - Run VidCap API:
     ```bash
     python .agent/skills/youtube-handling/scripts/vidcap.py info "<url>"
     python .agent/skills/youtube-handling/scripts/vidcap.py summary "<url>" --screenshot
     python .agent/skills/youtube-handling/scripts/vidcap.py caption "<url>" --ext vtt
     ```
...
3. **Design Infographic**
   ...
   - Inject brand context:
     ```bash
     node .agent/skills/brand-guidelines/scripts/inject-brand-context.cjs --json
     ```
...
<head>
  ...
  <style>
    ...
    :root {
      /* Paste output from: node .agent/skills/design-system/scripts/embed-tokens.cjs --minimal */
    }
  </style>
  <style>
    body {
