...
Reference: `.agent/skills/video-production/references/audio-directive-guide.md`
...
Reference: `.agent/skills/ai-multimodal/references/music-generation.md`
...
```bash
node .agent/skills/video-production/scripts/analyze-video.cjs \
...
```bash
node .agent/skills/video-production/scripts/optimize-for-platform.cjs \
...
```bash
node .agent/skills/video-production/scripts/extract-captions.cjs \
  --video "assets/videos/{date}-{slug}/master.mp4" \
  --format srt \
