...
- TypeScript (29 files)
- JavaScript (1 files)
- **Total:** 53 files, 19 directories
- **File types:** .ts: 29, .json: 5, .md: 5, .html: 3, .png: 2, .css: 2, .example: 1, .gitattributes: 1

## Public API / Exports
- `TtsProvider` from `src\config.ts`
- `VideoTheme` from `src\config.ts`
...
function floatDefault(name: string, def: number): number {
  ...
  if (isNaN(n)) throw new Error(`Env var ${na
```
