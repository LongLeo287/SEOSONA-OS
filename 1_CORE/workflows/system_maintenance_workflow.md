...
## PHASE 3: SYSTEM HEALTH AUDIT (WAVE 4+5 UPDATED)
*Objective: Ensure all system links, connections, and capability registries are intact.*

### Step 1: Health Diagnostic Commands
- Action: Run `npm run status:system` to verify OS integrity.
- Action: Run `npm run project:doctor` or `npm run seosona:doctor` to verify project connection state.

### Step 2: Capability & Router Check
- Action: Check `2_KNOWLEDGE/SKILLS_ROUTER.md` for any broken directory paths.
- Action: Cross-reference `4_AGENTS/AGENT_CAPABILITIES.md` to ensure all 49 agents have mapped skills.

### Step 3: Secrets & API Check
- Action: Ping configured MCP servers or APIs (run `npm run apis:free` or check `.vault`).
