...
function assertEquals(actual, expected, msg = '') {
  // Cross-platform: treat '\\' and '/' as equivalent path separators. These specs were authored
  // with POSIX '/'; on Windows the path utilities emit '\\'. Both are valid, so normalize string
  // operands before comparing. Non-strings (null, numbers) compare as-is, so real diffs still fail.
...
test('getGitRoot resolves through symlink to git repo', () => {
  ...
  try {
    ...
    // Create symlink (requires privilege/developer-mode on Windows — skip if unavailable)
    try { fs.symlinkSync(realDir, linkDir); }
    catch (e) { console.log('  (skipped: symlink creation not permitted here)'); return; }
...
test('getGitRoot with symlinked subdirectory', () => {
  ...
  try {
    ...
    // Create symlink to subdirectory (skip if the OS/privilege doesn't allow symlinks)
    try { fs.symlinkSync(subDir, linkToSub); }
    catch (e) { console.log('  (skipped: symlink creation not permitted here)'); return; }
