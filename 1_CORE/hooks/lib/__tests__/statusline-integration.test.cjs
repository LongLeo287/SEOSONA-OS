...
// These are INTEGRATION tests against the installed statusline at .claude/statusline.cjs.
// In a fresh checkout that artifact isn't installed, so skip cleanly rather than fail.
if (!fs.existsSync(path.join(process.cwd(), '.claude', 'statusline.cjs'))) {
  console.log('SKIPPED: statusline integration tests require an installed .claude/statusline.cjs (not present in this checkout).');
  process.exit(0);
}
...
function assertEquals(actual, expected, msg = '') {
  // Cross-platform: treat '\\' and '/' as equivalent path separators (POSIX-authored specs vs
  // Windows native paths). Non-strings compare as-is so real value diffs still fail.
