...
function sanitizePath(pathValue, projectRoot) {
  ...
  // For relative paths, resolve and validate. Resolve BOTH sides so the containment check is
  // consistent cross-platform (on Windows path.resolve rewrites a POSIX root to C:\..., so
  // comparing against the raw projectRoot wrongly rejected valid relative paths).
...
function getReportsPath(planPath, resolvedBy, planConfig, pathsConfig, baseDir = null) {
  ...
  // Return absolute path if baseDir provided. Normalize to forward slashes so the result is
  // stable across platforms (path.join emits '\\' on Windows); forward slashes work on Windows.
  if (baseDir) {
    return path.join(baseDir, reportPath).replace(/\\/g, '/');
  }
  return reportPath + '/';
}
