...
class KnowledgeGraph:
    ...
    def _scan_skill_files(self):
        ...
        for skill_md in SKILLS_DIR.rglob("SKILL.md"):
            ...
            # [Adaptive Chunking] Split document by markdown headers for semantic boundaries
            chunks = re.split(r'\n##+\s+', content)
            first_chunk = chunks[0] if chunks else ""

            # Extract description from the first semantic chunk
            desc_lines = []
            for line in first_chunk.splitlines()[1:]:
