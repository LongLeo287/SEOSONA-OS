...
7. For website-specific work, inspect `.openai/hosting.json` and treat `https://github.com/LongLeo287/SEOSONA` as the active SEOSONA Website source when no local website checkout is present.
8. For complex work, create `implementation_plan.md` and `task.md` before editing.
9. For narrow fixes, update `task.md` with the active checklist and proceed.
10. Validate with the smallest relevant command, and always run `npm run seosona:doctor` before connection handoff.
11. Log major milestones through:
