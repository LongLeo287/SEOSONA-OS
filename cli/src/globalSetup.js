...
function runSetup() {
    ...
    // Directory-based Agent injections (Mem0, Codex, SecureCoder, Ollama)
    function injectDirectoryAgent(name, dirPath) {
        const paddedName = `[${name}]`.padEnd(15, ' ');
    ...
    console.log("");
    console.log(colors.yellow("[2/2] Injecting into Background AI Services..."));
    injectDirectoryAgent("Mem0 Protocol", path.join(home, ".mem0"));
    injectDirectoryAgent("Ollama Local", path.join(home, ".ollama"));
    injectDirectoryAgent("Codex CLI", path.join(home, ".codex"));
    injectDirectoryAgent("SecureCoder", path.join(home, ".securecoder"));
